\chapter{一、任务定义与应用背景}\label{ux4e00ux4efbux52a1ux5b9aux4e49ux4e0eux5e94ux7528ux80ccux666f}

\section{1.1 任务定义}\label{ux4efbux52a1ux5b9aux4e49}
...
\section{1.2 应用背景}\label{ux5e94ux7528ux80ccux666f}
...
\chapter{二、相关工作调研}\label{ux4e8cux76f8ux5173ux5de5ux4f5cux8c03ux7814}

\section{2.1 RAG 与 GraphRAG}\label{rag-ux4e0e-graphrag}

Lewis 等人在 NeurIPS 2020 提出的 Retrieval-Augmented Generation
将生成模型与外部检索索引结合，证明了检索式非参数记忆能提升知识密集型任务的事实性和可追溯性
{[}1{]}。但标准 RAG
的基本单位通常是文本块，检索机制主要由查询与片段的向量相似度决定，难以表达文档内部的显式关系。

Edge 等人的 GraphRAG
将源文档构造成实体知识图谱，再对图社区生成摘要，用于回答局部和全局问题
{[}2{]}。微软官方文档进一步把查询模式分为 Local Search、Global
Search、DRIFT Search 和 Basic Search：Local Search
融合知识图谱与原文块，适合具体实体相关问题；Global Search
基于社区报告进行 map-reduce 式综合，适合整体性问题
{[}3{]}。本项目借鉴这一思想，但针对课程场景做轻量化改造：不完整复现微软的
Parquet 工作流、Leiden
层级社区报告和大规模全局总结，而是围绕单文档/小文档集构建本地 networkx
图，并在问答时生成图邻域与社区 profile 作为上下文。

2025 年 ACL Findings 的 Query-Driven Multimodal GraphRAG
指出，静态知识库和低效多模态融合会限制复杂跨模态推理；其方案根据查询语义动态构建局部图，采用多路径检索，并按需补充缺失的多模态信息
{[}9{]}。本项目目前采用``预构建文档图 +
查询时局部扩展''的折中路线：入库时建立稳定图谱，查询时再根据向量种子节点做一跳/二跳扩展，并补充页面图像。

\section{2.2
文档问答与多模态检索}\label{ux6587ux6863ux95eeux7b54ux4e0eux591aux6a21ux6001ux68c0ux7d22}

DocVQA 是文档视觉问答的经典数据集，包含约 12,000 张文档图像和 50,000
个问题，答案通常是从文档图像中抽取的文本 span
{[}4{]}。它强调模型不仅要读文字，还要理解表单、布局和文档结构。ChartQA
则聚焦图表问答，包含 9.6K 个人工编写问题和 23.1K
个由图表摘要生成的问题，问题常涉及视觉特征、逻辑推理和算术操作
{[}5{]}。这两个数据集分别覆盖文档
OCR/版面理解和图表推理，是本项目评测设计的重要依据。

ColPali 提出直接从文档页面图像生成多向量表示，并使用 late interaction
机制进行文档检索。该工作指出，文档页面包含表格、图像、版式和字体等视觉线索，传统文本抽取式检索流程脆弱且容易遗漏视觉信息
{[}6{]}。虽然本项目没有训练视觉检索器，但其思想支持我们保留页面原图，并在问答阶段将图像证据与文本/图谱证据一起输入
qwen3-vl-8b-instruct。

MMDocRAG 进一步说明了长文档多模态 RAG
的难度：不仅需要检索相关文本，还要选择和整合文本、表格、图像等多模态
quote，并评估证据链是否正确
{[}8{]}。因此，本项目的评测不只看最终答案，还会记录引用页码、节点
ID、图谱关系和失败案例。

\section{2.3
多模态模型选择}\label{ux591aux6a21ux6001ux6a21ux578bux9009ux62e9}

本项目使用 qwen3-vl-8b-instruct
作为文档解析和多模态回答的核心模型。Qwen3-VL
官方仓库和模型卡显示，该系列支持图像/视频/文本统一理解，增强了
OCR、空间感知、长上下文和多模态推理能力；Qwen3-VL-8B-Instruct 以
Apache-2.0 许可证开放，适合作为课程项目的低成本 API 或本地部署候选模型
{[}7{]}。相比传统 OCR + 版面分析 + 规则抽取流水线，VLM
直接对页面图片进行结构化抽取，可以更快搭建原型，并保留后续改进空间。

\section{2.4 方法对比}\label{ux65b9ux6cd5ux5bf9ux6bd4}
...
\bottomrule\noalign{}
\endlastfoot
纯文本 RAG & 实现简单，成本低 & 容易丢失图表、版面和跨页关系 &
作为消融或课程对照方向，不再作为主系统 \\
多模态 RAG & 能利用页面图像和图表信息 & 仍以相似片段为主，关系链不清晰 &
保留页面原图，并让 VLM 参与解析和生成 \\
文本 GraphRAG & 能利用实体/关系做多跳扩展 & 不处理视觉证据 &
借鉴图索引和局部/全局检索思想 \\
多模态 GraphRAG & 同时组织文本、视觉和关系证据 &
抽取噪声、图规模和上下文预算更难控制 & 当前主线方案：FAISS + networkx +
页面图像 + VLM \\
\end{longtable}

\chapter{三、系统方案设计}\label{ux4e09ux7cfbux7edfux65b9ux6848ux8bbeux8ba1}

\section{3.1 总体架构}\label{ux603bux4f53ux67b6ux6784}
...
\section{3.2 核心模块}\label{ux6838ux5fc3ux6a21ux5757}
...
\section{3.3
文档解析与结构化表示}\label{ux6587ux6863ux89e3ux6790ux4e0eux7ed3ux6784ux5316ux8868ux793a}
...
\section{3.4 图谱构建}\label{ux56feux8c31ux6784ux5efa}
...
\section{3.5 GraphRAG
检索与生成}\label{graphrag-ux68c0ux7d22ux4e0eux751fux6210}
...
\chapter{四、模型与技术选型}\label{ux56dbux6a21ux578bux4e0eux6280ux672fux9009ux578b}
...
\chapter{五、数据集与实验设计}\label{ux4e94ux6570ux636eux96c6ux4e0eux5b9eux9a8cux8bbeux8ba1}

\section{5.1 数据集计划}\label{ux6570ux636eux96c6ux8ba1ux5212}
...
\section{5.2 评测问题类型}\label{ux8bc4ux6d4bux95eeux9898ux7c7bux578b}
...
\section{5.3 指标设计}\label{ux6307ux6807ux8bbeux8ba1}
...
\section{5.4 实验组织}\label{ux5b9eux9a8cux7ec4ux7ec7}

项目主系统始终是多模态
GraphRAG，不再拆成``最低可交付版本''和``增强版本''。为了满足课程对实验分析的要求，后续将采用\textbf{同一系统内的消融开关}来解释各组件贡献：

\begin{itemize}
\tightlist
\item
  \textbf{Full：多模态 GraphRAG}，使用向量召回、图谱扩展、社区 profile
  和页面图像；
\item
  \textbf{No-Graph：关闭图谱扩展}，只使用向量召回节点和页面图像，观察多跳问题下降幅度；
\item
  \textbf{No-Image：关闭页面原图输入}，只使用文本和图谱证据，观察图表题、表格题下降幅度；
\item
  \textbf{Short-Answer Eval}：评测脚本使用 \texttt{ask\_eval()}
  约束输出为短答案，计算 ANLS/EM。
\end{itemize}

这种实验方式不把项目割裂为多个产品版本，而是在 GraphRAG
主线下分析``视觉证据''和``图结构证据''的独立贡献。

\chapter{六、算力预算与时间计划}\label{ux516dux7b97ux529bux9884ux7b97ux4e0eux65f6ux95f4ux8ba1ux5212}

\section{6.1 算力与成本}\label{ux7b97ux529bux4e0eux6210ux672c}
...
\section{6.2 当前进度}\label{ux5f53ux524dux8fdbux5ea6}
...
\section{6.3 后续计划}\label{ux540eux7eedux8ba1ux5212}
...
\chapter{七、风险分析与应对方案}\label{ux4e03ux98ceux9669ux5206ux6790ux4e0eux5e94ux5bf9ux65b9ux6848}
...
\bottomrule\noalign{}
\endlastfoot
VLM 结构化抽取不稳定 & JSON 缺字段、文本过稀疏、关系幻觉 & 已加入 repair
prompt、text-only pass、schema 归一化；后续继续加强关键词过滤 \\
图谱过密或噪声边过多 & 图扩展引入无关证据，影响回答 & 限制
\texttt{max\_nodes}，按节点类型和度排序邻居，弱化实体节点，优先内容锚点和关键词 \\
多文档同名实体/关键词混淆 & 检索跨错文档 & 已在元数据中保留
\texttt{source}，问答链支持
\texttt{source\_hint}；评测时按图片/文档源过滤 \\
API 成本和时间不可控 & 大量页面重复解析 & 使用文件 sha256
注册表和产物复用；评测目录独立设置 \texttt{OUTPUT\_ROOT} \\
图表题仍需视觉细节 & 文本描述不足以回答坐标/趋势问题 &
问答阶段保留页面原图和 bbox 预览，必要时让 VLM 直接看相关页面 \\
课程要求需要定量结果 & 只有 demo 说服力不足 & 已准备 DocVQA
评测脚本，后续补充 ANLS/EM、引用命中率和人工评分 \\
\end{longtable}

\chapter{八、创新点与贡献}\label{ux516bux521bux65b0ux70b9ux4e0eux8d21ux732e}
